 ...
 *  Returns
 *      CBX_E_NONE if element allocated successfully
 *      CBX_E_* as appropriate otherwise
 *  Notes
 *      No locking or parameter checking is performed.  This is used internally
 ...
 *  Returns
 *      CBX_E_NONE if element freed successfully
 *      CBX_E_* as appropriate otherwise
 *  Notes
 ...
 *   Returns
 *      CBX_E_NONE if list created successfully
 *      CBX_E_* as appropriate otherwise
 *   Notes
                        ...
                        char *name)
{
    ...
    /* FIXME: currently allocates one extra 'element' record as overrun chk */
    AIDXRES_DUMP(("shr_aidxres_list_create: Allocate %d byte cell for %s.\n",
                 ...
                 name));
    size = ((sizeof(_aidxres_list_t) - sizeof(_aidxres_list_sublist_t)) +
            (block_factor * (sizeof(_aidxres_list_sublist_t))) +
            ((banks * block_factor) * ((sizeof(_aidxres_list_bank_t)) + sizeof(_aidxres_list_bank_sublist_t))) +
            (count * sizeof(_aidxres_list_elemdesc_t)));
    work_list = (shr_aidxres_list_handle_t)sal_alloc(size, name);
    if (!work_list) {
 ...
 *      (in) shr_aidxres_element_t count = number of elements in block
 *   Returns
 *      bcm_error_t = CBX_E_NONE if elements reserved successfully
 *                    CBX_E_* as appropriate otherwise
 *   Notes
