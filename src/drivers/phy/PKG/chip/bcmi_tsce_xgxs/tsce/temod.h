...
typedef enum eagle_tsce_vco_enum {
    VCO_10p3125,
    VCO_12p5
} eagle_tsce_vco_enum_t;
...
extern int temod_pmd_lane_swap(PHYMOD_ST *pc, int lane_map);
...
extern int tsce_phy_pcs_speed_set(const phymod_access_t *pa, temod_spd_intfc_type speed);
extern int eagle_tsce_vco_setup(const phymod_access_t *pa, phymod_ref_clk_t ref_clk, eagle_tsce_vco_enum_t vco);

extern int tsce_phy_pcs_speed_get(const phymod_access_t *pa, uint32_t *speed);
#endif
