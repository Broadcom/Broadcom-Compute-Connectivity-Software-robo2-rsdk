...
err_code_t _eagle_tsc_pmd_mwr_reg_byte_addr32(const phymod_access_t *pa, uint32_t addr, uint16_t mask, uint8_t  lsb, uint8_t val) {

    EFUN(eagle_tsc_pmd_mwr_reg_addr32(pa,addr, mask, lsb, (uint16_t) val));

    return(ERR_CODE_NONE);
}

err_code_t _eagle_tsc_pmd_rdt_field_byte_addr32( const phymod_access_t *pa, uint32_t addr, uint8_t shift_left,   uint8_t shift_right, uint8_t *val8_p) {

      uint16_t val;
      EFUN(eagle_tsc_pmd_rdt_reg_addr32(pa,addr,&val));
      val <<= shift_left;                   /* Move the MSB to the left most      [shift_left  = (15-msb)]     */
      val >>= shift_right;                  /* Right shift entire field to bit 0  [shift_right = (15-msb+lsb)] */
...
err_code_t _eagle_tsc_pmd_rdt_field_signed_byte_addr32( const phymod_access_t *pa, uint32_t addr, uint8_t        shift_left, uint8_t shift_right, int8_t *val8_p) {
...
uint16_t _eagle_tsc_pmd_rde_reg_addr32(const phymod_access_t *pa, uint32_t addr, err_code_t *err_code_p)
{
    uint16_t data;
    EPFUN(eagle_tsc_pmd_rdt_reg_addr32(pa,addr, &data));
    return data;
}

uint16_t _eagle_tsc_pmd_rde_field_addr32(const phymod_access_t *pa, uint32_t addr, uint8_t shift_left, uint8_t shift_right, err_code_t *err_code_p)
{
      uint16_t data;
      EPSTM(data = _eagle_tsc_pmd_rde_reg_addr32( pa, addr, err_code_p));
...
err_code_t _eagle_tsc_pmd_rde_field_byte_addr32( const phymod_access_t *pa, uint32_t addr, uint8_t shift_left, uint8_t shift_right, err_code_t *err_code_p) {
       return ((uint8_t) _eagle_tsc_pmd_rde_field_addr32( pa, addr, shift_left, shift_right, err_code_p));
}
