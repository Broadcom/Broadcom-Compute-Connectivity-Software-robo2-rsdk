...
#define LEFUN(fun) do {int __err = (fun); if (__err) return(__err); } while(0)
...
int temod_pmd_lane_swap(PHYMOD_ST *pc, uint32_t lane_map);
 ...
 /* if((an_ability_st->cl37_adv.an_type ==  TEMOD_AN_MODE_CL37) || ( an_ability_st->cl37_adv.an_type ==  TEMOD_AN_MODE_CL37BAM) ||
  ...
  /*if((an_ability_st->cl73_adv.an_type ==  TEMOD_AN_MODE_CL73) || ( an_ability_st->cl73_adv.an_type ==  TEMOD_AN_MODE_CL73BAM)) */  {
      ...
      /* CL72 Enable logic
         ...
         FIXME: if cl72_en=1, we clear all overrides. Need to add logic to just disable
         cl72 field override
      */
...
int temod_autoneg_control_get(PHYMOD_ST* pc, temod_an_control_t *an_control, int *an_complete)
{
    ...
    /* FIXME : Only one an_property_tpe is returned type, so the sgmii maybe replaced by the code later in this tier1 */
...
int temod_autoneg_remote_ability_get(PHYMOD_ST* pc, temod_an_ability_t *an_ability_st)
{
  ...
  /* FIXME - For now it is fine, because teh Local would be same as remote. DOUBLE CHECK THIS CODE */
...
#ifdef _SDK_TEMOD_
int temod_refclk_set(PHYMOD_ST* pa, temod_ref_clk_t refclk)
{
...
{
  DIG_RST_CTL_PMDr_t reg_rst_ctrl_pmd;
  CKRST_LN_S_RSTB_CTLr_t reg_rstb_ctrl;

  TEMOD_DBG_IN_FUNC_INFO(pc);
  TEMOD_DBG_IN_FUNC_VIN_INFO(pc,("pmd_touched: %x", pmd_touched));
  DIG_RST_CTL_PMDr_CLR(reg_rst_ctrl_pmd);
  CKRST_LN_S_RSTB_CTLr_CLR(reg_rstb_ctrl);

  if (pmd_touched == 0) {
    DIG_RST_CTL_PMDr_CORE_S_RSTBf_SET(reg_rst_ctrl_pmd,1);
    PHYMOD_IF_ERR_RETURN(WRITE_DIG_RST_CTL_PMDr(pc,reg_rst_ctrl_pmd));
    CKRST_LN_S_RSTB_CTLr_LN_S_RSTBf_SET(reg_rstb_ctrl,1);
    PHYMOD_IF_ERR_RETURN(WRITE_CKRST_LN_S_RSTB_CTLr(pc, reg_rstb_ctrl));
  }
  return PHYMOD_E_NONE;
}
...
int temod_plldiv_lkup_get(PHYMOD_ST* pc, temod_spd_intfc_type spd_intf, uint32_t *plldiv, uint16_t *speed_vec)
{
  ...
  *speed_vec = speed_id ;
  TEMOD_DBG_IN_FUNC_VOUT_INFO(pc,("plldiv: %d", (int)*plldiv));
...
@param   pc handle to current TSCE context (#PHYMOD_ST)
@param   real_check FIXME
@param   prbs_status FIXME
...
@param   pc handle to current TSCE context (#PHYMOD_ST)
@param   port FIXME
@param   prbs_inv FIXME
@param   pattern_type FIXME
@param   prbs_check_mode FIXME
@returns The value PHYMOD_E_NONE upon successful completion
...
/* FIXME : setting PCS in bypass mode */
  TEMOD_DBG_IN_FUNC_INFO(pc);
...
int temod_prbs_control(PHYMOD_ST* pc, int prbs_enable)                /* PRBS_CONTROL */
{
  ...
  TLB_RX_PRBS_CHK_CFGr_CLR(reg_prbs_chk_cfg);
  /*  FIXME
  prbs_rx_en = ((pc->per_lane_control & 0xf00) >> 8) ? 1: 0 ;
  prbs_tx_en = (pc->per_lane_control & 0xf) ? 1 : 0 ;
*/
...
#ifdef _DV_TB_
  /*FIXME Check if teh SDK needs more info on failure */
...
int temod_pmd_lane_swap(PHYMOD_ST *pc, int lane_map)
{
//   int __err;
...
#ifdef _SDK_TEMOD_
/* IMPORTANT FIXME SHANTA for adjust_port_mode */
#endif
...
/*FIXME, can remove this once cl72 is part of regular flow testing. Just for AN
...
/* FIXME: init_pcs_an, may not be used by sdk, can take this ofif?  */
int _init_pcs_an(PHYMOD_ST* pc)
{
