/*----------------------------------------------------------------------
 * $Id: viper_pmd_cfg_seq.h,v 1.1.2.2 2013/09/17 21:11:10 wniu Exp $
 * 
...
typedef enum {
    ...
    VIPER_SPD_1000                   ,  /*!< 1G  based on 1000BASE-X         */
...
extern int viper_sgmii_master_aneg_100M (const PHYMOD_ST *pa);
...
extern int viper_autoneg_set(PHYMOD_ST* pa, const phymod_autoneg_control_t* an);
...
extern int viper_xgxs_sel(const PHYMOD_ST *pa, int select_val);
...
extern int viper_quad_gmii_enable (const PHYMOD_ST *pa);
extern int viper_xgxs_sel_enable(PHYMOD_ST *pa, uint32_t enable);
extern int viper_pll_disable_qsgmii_single_lane (PHYMOD_ST *pa);
extern int viper_disable_lmtcal(const PHYMOD_ST *pa);
extern int viper_rx_large_swing_set(PHYMOD_ST *pa, uint32_t enable);
extern int viper_disable_carrier_extend(PHYMOD_ST *pa);
extern int viper_pll_afe_config_refclk156 (PHYMOD_ST *pa);
extern int viper_pll_afe_config_refclk156_6p25Ghz (PHYMOD_ST *pa);
extern int viper_qsgmii_enable (const PHYMOD_ST *pa);
extern int viper_pll_enable_qsgmii_single_lane_refclk156 (PHYMOD_ST *pa);
