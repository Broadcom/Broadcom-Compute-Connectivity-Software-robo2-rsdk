/*
 *
 * $Id: viper_pmd_cfg_seq.c,v 1.2.2.26 2013/09/16 12:23:15 hoangngo Exp $
 *
...
int viper_pll_afe_config_refclk156_6p25Ghz (PHYMOD_ST *pa)
{
    ...
    //PLL_AFE_CTL4r_SET(pll_afectrl4, 0x8021); /* Provided in Doc, not clear */
    PLL_AFE_CTL4r_SET(pll_afectrl4, 0x0021);
    WRITE_PLL_AFE_CTL4r(pa, pll_afectrl4);
...
int viper_pll_afe_config_refclk156 (PHYMOD_ST *pa)
{
    /* PLL AFE Programming */
    PLL_AFE_CTL0r_t pll_afectrl0;
    PLL_AFE_CTL1r_t pll_afectrl1;
...
int viper_fiber_force_2p5G (PHYMOD_ST *pa)
{
    ...
    /* check for PLL stayed lock - workaround for Avenger-498 */
    PHYMOD_USLEEP(10000);
    PHYMOD_USLEEP(10000);
 ...
 *
 */
int viper_sgmii_master_aneg_100M (const PHYMOD_ST *pa)
{
...
int viper_rx_pol_set (const PHYMOD_ST *pa, uint8_t val)
{
    ...
    /* FIXME Only turn on this bit, the first lane link is up*/
...
int viper_autoneg_set(PHYMOD_ST* pa, const phymod_autoneg_control_t* an)
{
    MIICTLr_t         miictl;
    DIG_CTL1000X1r_t  x1reg;
    DIG_CTL1000X2r_t  x2reg;
...
#if 0
    int rv;
    /* FIXME, XGXS has reg 0x8001 or not, bcmi_viper_xgxs_defs.h doesn't have such reg*/
...
int viper_set_spd_intf(PHYMOD_ST *pa, viper_spd_intfc_type_t type)
{
    ...
    switch(type){
        ...
        case VIPER_SPD_AN_SGMII_M_100M:
            PHYMOD_IF_ERR_RETURN(viper_sgmii_master_aneg_100M(pa));
            break;
...
int viper_multimmds_set(PHYMOD_ST *pa, int enable)
{
    ...
    MMDSELr_MULTIPRTS_ENf_SET(mreg, 0);
   ...
   XGXSSTS1r_t xreg;
    ...
    if (pa->lane_mask == 0xf) {
        ...
        for (lane = 0; lane < 4; lane++){
            ...
            READ_DIG_STS1000X1r(pa, &sreg);
            speed_id = DIG_STS1000X1r_SPEED_STATUSf_GET(sreg);
            if (speed_id == VIPER_SPD_10M) {
                speed = 10;
            } else if(speed_id == VIPER_SPD_100M) {
                speed = 100;
            } else if(speed_id == VIPER_SPD_1G) {
                speed = 1000;
            } else if(speed_id == VIPER_SPD_2p5G) {
                speed = 2500;
            }
