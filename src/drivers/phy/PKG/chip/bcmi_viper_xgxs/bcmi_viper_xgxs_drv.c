...
_viper_ability_set(phy_ctrl_t *pc, uint32_t ability)
{
    ...
    autoneg_ability.capabilities = 0;
...
_viper_ability_get(phy_ctrl_t *pc, uint32_t *ability)
{
    ...
    _viper_phymod_phy_t_init(pc, &pm_phy);
    CDK_MEMSET(&phymod_autoneg_ability, 0, sizeof(phymod_autoneg_ability_t));
...
bcmi_viper_xgxs_init(phy_ctrl_t *pc)
{
    ...
    for (idx = 0; idx < pc->num_phys; idx++) {
        ...
        phy_init_config.polarity.rx_polarity = 0;
        phy_init_config.polarity.tx_polarity = 0;
...

        {
                ...
                if (if_cfg->interface_type == phymodInterfaceSGMII) {
                    if_cfg->data_rate = 2500;
                    if_cfg->interface_modes = 0;
#if (CONFIG_SGMII_SPEED == 1000)
                    if_cfg->data_rate = 1000;
                    if_cfg->interface_modes = 1;
#endif
                } else if(if_cfg->interface_type == phymodInterface1000X) {
                    ...
                    if_cfg->data_rate = 2500;
#endif
                } else { /* phymodInterfaceQSGMII */
                    if_cfg->data_rate = 10000;
                }
                inst = PHY_CTRL_INST(pc) & LANE_NUM_MASK;
                pm_phy.access.lane_mask = 1 << inst;
                rv = viper_phy_interface_config_set(&pm_phy, 0, if_cfg);
        }
        ...
        if (CDK_SUCCESS(rv)) {
            ...
            /* Fix for odd numbered packet counter issue on port 9 */
            _viper_phymod_phy_t_init(pc, &pm_phy);
            pm_phy.access.lane_mask = 0x2;
            rv = viper_phy_disable_carrier_extend(&pm_phy);
#if 0
            if (pm_phy.access.lane_mask & 0x2) {
                rv = viper_phy_disable_carrier_extend(&pm_phy);
            }
#endif
        }
...
bcmi_viper_xgxs_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    ...
    *abil = PHY_ABIL_10MB | PHY_ABIL_100MB | PHY_ABIL_1000MB |
            PHY_ABIL_2500MB | PHY_ABIL_LOOPBACK |
    ...
    case PhyStatus_LineInterface:
        {
            ...
            _viper_phymod_phy_t_init(pc, &pm_phy);
            if (CDK_SUCCESS(rv)) {
                ioerr = viper_phy_interface_config_get(&pm_phy, 0, ref_clock,
                                                   &interface_config);
            }
...
static int
bcmi_viper_xgxs_reg_read(phy_ctrl_t *pc, uint32_t addr,  uint32_t *val)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;

    _viper_phymod_phy_t_init(pc, &pm_phy);
    rv = viper_phy_reg_read(&pm_phy, addr,  val);
    return rv;
}

static int
bcmi_viper_xgxs_reg_write(phy_ctrl_t *pc, uint32_t addr, uint32_t val)
{
    int rv = CDK_E_NONE;
    phymod_phy_access_t pm_phy;
    _viper_phymod_phy_t_init(pc, &pm_phy);
    rv = viper_phy_reg_write(&pm_phy, addr, val);
    return rv;
}
...
phy_driver_t bcmi_viper_xgxs_drv = {
    ...
    NULL,                                 /* pd_cable_diag */
    bcmi_viper_xgxs_reg_read,             /* pd_reg_read */
    bcmi_viper_xgxs_reg_write,            /* pd_reg_write */
