 ...
 *
 * PHY driver for BCM542xx.
 *
 */

#include <phy/phy.h>
#include <phy/ge_phy.h>
#include <phy/chip/bcm542xx_defs.h>

#define BCM54210_PHY_ID0                0x600d
#define BCM54210_PHY_ID1                0x8590

#define BCM54291_PHY_ID0                0x600d
#define BCM54291_PHY_ID1                0x8530
...
/* Default LED control */
#define BCM542xx_LED1_SEL(_pc)          0x0
#define BCM542xx_LED2_SEL(_pc)          0x1
#define BCM542xx_LED3_SEL(_pc)          0x3
#define BCM542xx_LED4_SEL(_pc)          0x6
#define BCM542xx_LEDCTRL(_pc)           0x8
#define BCM542xx_LEDSELECT(_pc)         0x0

#define PHY_IS_BCM54210(pc) \
                ((phyid0 == BCM54210_PHY_ID0) && \
                 ((phyid1 & ~PHY_ID1_REV_MASK) == BCM54210_PHY_ID1) && \
                 !((phyid1 & PHY_ID1_REV_MASK) & 0x8))

#define PHY_IS_BCM54291(pc) \
                ((phyid0 == BCM54291_PHY_ID0) && \
                 ((phyid1 & ~PHY_ID1_REV_MASK) == BCM54291_PHY_ID1) && \
                 ((phyid1 & PHY_ID1_REV_MASK) & 0x8))

#define PHY_IS_BCM54292(pc) \
                ((phyid0 == BCM54292_PHY_ID0) && \
...
extern cdk_symbols_t bcm542xx_symbols;
#endif
 ...
 */
static int
bcm542xx_phy_probe(phy_ctrl_t *pc)
{
    ...
    if (PHY_IS_BCM54291(pc) || PHY_IS_BCM54292(pc) ||
        PHY_IS_BCM54210(pc)) {
#if PHY_CONFIG_INCLUDE_CHIP_SYMBOLS == 1
        PHY_CTRL_SYMBOLS(pc) = &bcm542xx_symbols;
#endif
 ...
 */
static int
bcm542xx_phy_notify(phy_ctrl_t *pc, phy_event_t event)
{
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_reset(phy_ctrl_t *pc)
{
    int rv;
 ...
 */
static int
bcm542xx_phy_init(phy_ctrl_t *pc)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    ...
    orig_inst = _bcm542xx_inst(pc);

    /* Switch to instance 0 to enable QSGMII MDIO sharing */
    if (phy_ctrl_change_inst(pc, 0, _bcm542xx_inst) < 0) {
        return CDK_E_FAIL;
    }
    ...
    if (phy_ctrl_change_inst(pc, 3, _bcm542xx_inst) < 0) {
        return CDK_E_FAIL;
    }
    ...
    /* Restore instance */
    phy_ctrl_change_inst(pc, orig_inst, _bcm542xx_inst);
    ...
    led_val = BCM542xx_LED1_SEL(pc) | (BCM542xx_LED2_SEL(pc) << 4);
    LED_SELECTOR_1r_SET(led_sel_1, led_val);
    ioerr += WRITE_LED_SELECTOR_1r(pc, led_sel_1);

    led_val = BCM542xx_LED3_SEL(pc) | (BCM542xx_LED4_SEL(pc) << 4);
    LED_SELECTOR_2r_SET(led_sel_2, led_val);
    ioerr += WRITE_LED_SELECTOR_2r(pc, led_sel_2);

    LED_CTRLr_SET(led_ctrl, BCM542xx_LEDCTRL(pc));
    ioerr += WRITE_LED_CTRLr(pc, led_ctrl);

    EXP_LED_SELECTORr_SET(exp_led_selector, BCM542xx_LEDSELECT(pc));
 ...
 */
static int
bcm542xx_phy_link_get(phy_ctrl_t *pc, int *link, int *autoneg_done)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_duplex_set(phy_ctrl_t *pc, int duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_duplex_get(phy_ctrl_t *pc, int *duplex)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t speed;
 ...
 */
static int
bcm542xx_phy_speed_set(phy_ctrl_t *pc, uint32_t speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_speed_get(phy_ctrl_t *pc, uint32_t *speed)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_autoneg_set(phy_ctrl_t *pc, int autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
 ...
 */
static int
bcm542xx_phy_autoneg_get(phy_ctrl_t *pc, int *autoneg)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    uint32_t speed;
 ...
 */
static int
bcm542xx_phy_loopback_set(phy_ctrl_t *pc, int enable)
{
    int ioerr = 0;
    int rv = CDK_E_NONE;
    int ge_lb, fe_lb;
 ...
 */
static int
bcm542xx_phy_loopback_get(phy_ctrl_t *pc, int *enable)
{
    int ioerr = 0;
    MII_1000X_CTRLr_t mii_1000x_ctrl;
    COPPER_MISC_TESTr_t copper_misc_test;
 ...
 */
static int
bcm542xx_phy_ability_get(phy_ctrl_t *pc, uint32_t *abil)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm542xx_phy_config_set(phy_ctrl_t *pc, phy_config_t cfg, uint32_t val, void *cd)
{
    PHY_CTRL_CHECK(pc);
    ...
    case PhyConfig_TempMon: {
        ...
        /* Restore instance */
        (void)phy_ctrl_change_inst(pc, orig_inst, _bcm542xx_inst);
 ...
 */
static int
bcm542xx_phy_config_get(phy_ctrl_t *pc, phy_config_t cfg, uint32_t *val, void *cd)
{
    PHY_CTRL_CHECK(pc);
 ...
 */
static int
bcm542xx_phy_status_get(phy_ctrl_t *pc, phy_status_t st, uint32_t *val)
{
    PHY_CTRL_CHECK(pc);
...
/*
 * Variable:    bcm542xx_phy drv
 * Purpose:     PHY Driver for BCM542xx.
 */
phy_driver_t bcm542xx_drv = {
    "bcm542xx",
    "BCM542xx Gigabit PHY Driver",  
    0,
    bcm542xx_phy_probe,                 /* pd_probe */
    bcm542xx_phy_notify,                /* pd_notify */
    bcm542xx_phy_reset,                 /* pd_reset */
    bcm542xx_phy_init,                  /* pd_init */
    bcm542xx_phy_link_get,              /* pd_link_get */
    bcm542xx_phy_duplex_set,            /* pd_duplex_set */
    bcm542xx_phy_duplex_get,            /* pd_duplex_get */
    bcm542xx_phy_speed_set,             /* pd_speed_set */
    bcm542xx_phy_speed_get,             /* pd_speed_get */
    bcm542xx_phy_autoneg_set,           /* pd_autoneg_set */
    bcm542xx_phy_autoneg_get,           /* pd_autoneg_get */
    bcm542xx_phy_loopback_set,          /* pd_loopback_set */
    bcm542xx_phy_loopback_get,          /* pd_loopback_get */
    bcm542xx_phy_ability_get,           /* pd_ability_get */
    bcm542xx_phy_config_set,            /* pd_config_set */
    bcm542xx_phy_config_get,            /* pd_config_get */
    bcm542xx_phy_status_get,            /* pd_status_get */
    NULL,                               /* pd_cable_diag */
    NULL,                               /* pd_reg_read */
    NULL,                               /* pd_reg_write */
