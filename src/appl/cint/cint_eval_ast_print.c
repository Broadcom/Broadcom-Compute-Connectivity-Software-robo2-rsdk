...
cint_eval_ast_Print(cint_ast_t* ast)
{
    ...
    if(ast) {
       /* Null check when cint print invoked with invalid format */
       if(!(ast->utype.print.expression)) {
           cint_ast_error(NULL, CINT_E_BAD_EXPRESSION, "unrecognized print format");
           return v;
       }
